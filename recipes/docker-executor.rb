...
version = '0.19.0_rc2' if version == '0.19.0'
...
# this doesn't work. so we have to install docker manually outside. I can't
# figure out why.
...
remote_file "#{Chef::Config[:file_cache_path]}/#{egg_name}.egg" do
  source 'http://downloads.mesosphere.io/master/ubuntu/' \
         "#{platform_version}/#{egg_name}.egg"
  mode 0755
  not_if do
    File.exist?("#{Chef::Config[:file_cache_path]}/#{egg_name}.egg") == true
  end
...
execute 'install-mesos-python-binding' do
  ...
  not_if { ::File.exist?("/usr/local/lib/python2.7/dist-packages/#{egg_name}.egg") }
end
...
remote_file '/var/lib/mesos/executors/docker' do
  ...
  mode 0755
  not_if { File.exist?('/var/lib/mesos/executor/docker') == true }
end
